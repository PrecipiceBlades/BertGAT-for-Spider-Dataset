...
class MultiSqlPredictor(nn.Module):
    ...
    def forward(self, q_emb_var, q_len, hs_emb_var, hs_len, mkw_emb_var, mkw_len, syn_graph, syn_feat, syn_len):
        # print("q_emb_shape:{} hs_emb_shape:{}".format(q_emb_var.size(), hs_emb_var.size()))
        max_q_len = max(q_len)
        max_hs_len = max(hs_len)
        max_syn_len = max(syn_len)
        B = len(q_len)
        #print ('syn_feat is cuda', syn_feat.is_cuda)
        ...
        if self.use_syn:
            ...
            att_syn_qmkw = torch.bmm(mkw_enc, self.q_att(syn_enc).transpose(1, 2))
        ...
        #print(q_enc.shape, hs_enc.shape, mkw_enc.shape, syn_enc.shape)
        ...
        #print (q_weighted.shape, mkw_enc.shape, syn_enc.shape)
        if self.use_syn:
            mulit_score = self.multi_out(self.multi_out_q(q_weighted) + int(self.use_hs)* self.multi_out_hs(hs_weighted) \
                                         + self.multi_out_c(mkw_enc) +  self.multi_out_syn(syn_weighted)).view(B,-1)
        else:
            mulit_score = self.multi_out(self.multi_out_q(q_weighted) + int(self.use_hs)* self.multi_out_hs(hs_weighted) \
