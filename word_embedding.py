...
class WordEmbedding(nn.Module):
    ...
    def gen_x_syn_batch(self, q, syntax_data):
        B = len(q)
        feat_val_embs = []
        syn_graph = []
        ...
        for i, one_q in enumerate(q):
            feat_val = []
            comb_q = ' '.join(one_q)
            sent = syntax_data[comb_q]['sentence']
            edge = syntax_data[comb_q]['edge']
            g = dgl.DGLGraph()
            g.add_nodes(len(sent))
            if edge:
            ...
            syn_graph.append(g)
            for ws in sent:
                feat_val.append(self.word_emb.get(ws, np.zeros(self.N_word, dtype=np.float32)))

            val_len[i] = len(sent)
            #print(feat_val, len(feat_val))
            feat_val_embs.append(feat_val)

        max_len = max(val_len)

        val_emb_array = np.zeros((B, max_len, self.N_word), dtype=np.float32)
        for i in range(B):
            #print(i, len(feat_val_embs))
            for t in range(len(feat_val_embs[i])):
                val_emb_array[i, t, :] = feat_val_embs[i][t]
        syn_feats = torch.from_numpy(val_emb_array)
        if self.gpu:
            syn_feats = syn_feats.cuda()
        syn_feats_var = Variable(syn_feats)

        return syn_graph, syn_feats, val_len

    def gen_x_q_batch_old(self, q):
        B = len(q)
        val_embs = []
        val_len = np.zeros(B, dtype=np.int64)
        for i, one_q in enumerate(q):
    ...
    def gen_x_q_batch(self, q, qs):
        B = len(q)
        val_embs_new = []
        val_len_new = np.zeros(B, dtype=np.int64)
        for i, one_q in enumerate(q):
            q_val = []
            q_val_new = []
            temp = qs[one_q]
            for j in np.arange(temp.shape[0]):
                q_val_new.append(temp[j,:])
            val_embs_new.append(q_val_new)
            val_len_new[i] = len(q_val_new)
        max_len_new = max(val_len_new)

        val_emb_array_new = np.zeros((B, max_len_new, 768), dtype=np.float32)
        for i in range(B):
            for t in range(len(val_embs_new[i])):
                val_emb_array_new[i, t, :] = val_embs_new[i][t]
        val_inp_new = torch.from_numpy(val_emb_array_new)
        if self.gpu:                                                                        val_inp_new = val_inp_new.cuda()
        val_inp_var_new = Variable(val_inp_new)
        return val_inp_var_new, val_len_new
